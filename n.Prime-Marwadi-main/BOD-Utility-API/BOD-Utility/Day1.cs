    ...
    internal static class Day1
    {
            ...
            ConcurrentDictionary<string, ContractMaster> dict_TokenScripInfo)
        {
            ...
            try
            {
                ...
                try
                {
                    var Day1File = Day1Directory.GetFiles("Prime*").OrderByDescending(f => f.LastWriteTime).First();
                    ...
                    foreach (var _Line in arr_Day1)
                    {
                        ...
                        try
                        {
                            ...
                            if (!dict_CustomScripInfo.TryGetValue(CustomScripNameKey, out ScripInfo))
                            {
                                ...
                                if (!dict_CustomScripInfo.TryGetValue(CustomScripNameKey, out ScripInfo))
                                {
                                    _logger.Debug("ReadDAY1 Skipped :  " + CustomScripName);
                                    continue;
                                }
                            }
                ...
                try
                {
                    ...
                    foreach (var _Line in arr_Day1)
                    {
                        ...
                        catch (Exception ee) { _logger.Debug("Day1 Loop : " + _Line + Environment.NewLine + ee); isAnyError = true; }
                    }
                }
                catch (Exception ee) { _logger.Error(ee); }
