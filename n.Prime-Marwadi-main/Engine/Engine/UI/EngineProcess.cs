...
using en_Segment = FeedLibrary.Data_Structures.en_Segment;
...
namespace Engine
{
    public partial class EngineProcess : XtraForm
    {
        ...
        double _FOLastTradeTime = 0, _CMLastTradeTime = 0, isGatewayConnected = 0, isSpanConnected = 0, FOLTT = 0, CMLTT = 0, _SpanComputeTime = 0;
        ...
        /// list of CM bhavcopy
        /// </summary>
        List<Exchange.CMBhavcopy> list_CMBhavcopy = new List<Exchange.CMBhavcopy>();
        ...
        private void ReadContractAndFill()
        {
            try
            {
                ...
                var ClosestUnderlyingTokens = list_Contract.Where(v => v.ScripType == Exchange.en_ScripType.XX).Select(v => new { v.Token, v.Symbol }).GroupBy(g => new { g.Symbol }).Select(r => new { Token = r.Select(x => x.Token).First(), Underlying = r.Select(x => x.Symbol).First() }).ToList();
                ...
                foreach (var item in ClosestUnderlyingTokens)
                {
                    try
                    {
                        ...
                        var dict_Strikes = Special.Where(v => v.Symbol.Equals(item.Underlying) && v.Expiry.Equals(Expiry) && v.ScripType != Exchange.en_ScripType.XX).Select(v => v.StrikePrice).Distinct().ToDictionary(k => k, v => arr_CEPETokens);
                ...
                foreach (var item in list_Contract.Where(v => v.ScripType != Exchange.en_ScripType.XX))
                {
                    //added on 19NOV2020 by Amey
                    var _greeks = new Greeks();

                    var UnderlyingKey = $"{FeedLibrary.Data_Structures.en_Segment.NSEFO}|{item.Symbol}";
        ...
        private void ReceiveSpanFromGateway(string flag, Dictionary<string, double[]> dict_Span)
        {
            try
            {
                if (flag == "ALL")
                {
                    ...
                    if (isDebug)
                        _logger.WriteLog("Span Received Rows," + dict_SpanMargin.Count(), isDebug);
        ...
        private void ReadEODTable()
        {
            try
            {
                ...
                {
                    Username = v.Username,
                    Segment = v.Segment == "NSECM" ? n.Structs.en_Segment.NSECM : (v.Segment == "NSECD" ? n.Structs.en_Segment.NSECD :
                        (v.Segment == "NSEFO" ? n.Structs.en_Segment.NSEFO : n.Structs.en_Segment.BSECM)),
                    Token = v.Token,
                    ...
                    UnderlyingSegment = v.UnderlyingSegment == "NSECM" ? n.Structs.en_Segment.NSECM : (v.Segment == "NSECD" ? n.Structs.en_Segment.NSECD :
                        (v.Segment == "NSEFO" ? n.Structs.en_Segment.NSEFO : n.Structs.en_Segment.BSECM)),
                ...
                if (StartFromZero || isNewTrades)
                {
                    ...
                    foreach (var Position in list_CurrentPositions)
                    {
                        try
                        {
                            ...
                            if (_PositionInfo.ScripType == en_ScripType.EQ && dict_VARMargin.TryGetValue(_PositionInfo.ScripName, out VARMARGIN))
                            {
                        ...
                        {
                            _logger.WriteLog("Inner Linq Loop : " + innerLinq.ToString());
                        }
                    }
                ...
                else
                {
                    ...
                    foreach (var ClientID in dict_ConsolidatedPos.Keys)
                    {
                        ...
                        foreach (var _PositionInfo in dict_ConsolidatedPos[ClientID])
                        {
                            ...
                            if (_PositionInfo.Segment == n.Structs.en_Segment.NSECM)
                            {
                                //Added by Akshay For Collateral
            ...
            try
            {
                ...
                if (dict_LTP.TryGetValue(UnderlyingKey, out arr_LTP))
                {
                    if (arr_LTP[0] != -1)
                    {
                        ...
                        //added on 19NOV2020 by Amey
                        if (hs_CashTokens.Contains(ScripInfo.UnderlyingToken) && _ScripType != en_ScripType.EQ)
                ...
                if (_ScripType == en_ScripType.CE || _ScripType == en_ScripType.PE)
                {
                ...
                else if (_ScripType == en_ScripType.XX)
                {
                ...
                //added 0 check on 22APR2021 by Amey
        ...
        private void InsertDay1()
        {
            try
            {
                ...
                foreach (var _Item in result)
                {
                    ...
                    if ((ScripInfo.ExpiryUnix) > date_Tick || ScripInfo.ScripType == en_ScripType.EQ)   //09-01-18
        ...
        private void ReadCMBhavcopy()
        {
            try
            {
                ...
                list_CMBhavcopy = Exchange.ReadCMBhavcopy(CMBhavcopy.FullName);
        ...
        private void ReadConfig()
        {
            ...
            //var FilePath = "C:\\BOD_FILES\\20230628\\nsccl.20230628.i01.spn";
            //XmlTextReader _treader = new XmlTextReader(FilePath);
            //_treader.Read();
            //ds_Config.ReadXml(_treader);
            ...
            try
            {
                ...
                foreach (ds_Engine.dt_ContractMasterRow v in dt_ContractMaster.Rows)
                {
                    ScripInfo = new ContractMaster()
                    {
                        ...
                        LotSize = v.LotSize,
                        UnderlyingToken = v.UnderlyingToken,
                        UnderlyingSegment = v.UnderlyingSegment == "NSECM" ? n.Structs.en_Segment.NSECM : (v.UnderlyingSegment == "NSECD" ? n.Structs.en_Segment.NSECD :
                        (v.UnderlyingSegment == "NSEFO" ? n.Structs.en_Segment.NSEFO : n.Structs.en_Segment.BSECM))
                    };
                    ...
                    //added on 19APR2021 by Amey
                    if (ScripInfo.Segment == n.Structs.en_Segment.NSECM) //&& ScripInfo.Series == "EQ")
                    {
                    ...
                    //added on 20APR2021 by Amey
                    else if (ScripInfo.Segment == n.Structs.en_Segment.NSEFO)
        ...
        private void SendSpanToPrime()
        {
            try
            {
                ...
                if (dict_SpanData.Any())
                {

                    _EngineDataServer.SendSpanData(dict_SpanData);
                }
        ...
        private void _FeedLibrary_eve_BSECMTickReceived(BSECMPacket _BSECMPacket)
        {
            try
            {
                ...
                int Token = Convert.ToInt16(_BSECMPacket.Token);
                double LTP = Convert.ToDouble(_BSECMPacket.LTP);
                ...
                if (dict_LTP.TryGetValue(LTPKey, out arr_LTP))
                {
                    ...
                    //added on 15OCT2020 by Amey
                    //To identify whether received packet is Live or From Bhavcopy.
                    if (arr_LTP[2] == 0 && Convert.ToInt16(_BSECMPacket.LTQ) != 0)
                        arr_LTP[2] = 1;
                }
        ...
        private void _GatewayHeartBeatClient_eve_LTTReceived(double _FOLastTradeTime, double _CMLastTradeTime)
        {
            this._FOLastTradeTime = _FOLastTradeTime;
            this._CMLastTradeTime = _CMLastTradeTime;
        }

        private void _GatewayHeartBeatClient_eve_SpanStatusReceived(bool isSpanConnected)
        {
        ...
        private void _GatewayHeartBeatClient_eve_SpanInfoReceived(double _SpanComputeTime, string _LatestSpanFileName)
        {
