...
namespace Engine
{
    public delegate void del_LTTReceived(double _FOLastTradeTime, double _CMLasetTradeTime);
    public delegate void del_GatewayStatusReceived(bool isGatewayConnected);
    public delegate void del_SpanStatusReceived(bool isSpanConnected);
    public delegate void del_Error(string _Message);
    public delegate void del_SpanInfoReceived(double _SpanComputeTime, string _LatestSpanFileName);
                ...
                try
                {
                    ...
                    if (_ReceivedBytesLength > 0)
                    {
                        ...
                        if (_ReceivedFieldsLength > 1)
                        {
                            //added on 25JAN2021 by Amey
                            eve_GatewayStatusReceived(true);
                            eve_LTTReceived(Convert.ToDouble(arr_Fields[0]), Convert.ToDouble(arr_Fields[1]));
                        }

                        if (_ReceivedFieldsLength > 2)
                            eve_SpanStatusReceived(Convert.ToBoolean(arr_Fields[2]));

                        if (_ReceivedFieldsLength > 4)
                            eve_SpanInfoReceived(Convert.ToDouble(arr_Fields[3]), arr_Fields[4]);
                    }
                    else
