    ...
    public class clsWriteLog
    {
        ...
        public clsWriteLog(string path)
        {
            try
            {
                ...
                if (sw_Error == null)
                    sw_Error = File.AppendText(filename);
        ...
        public void WriteLog(string message, bool isDebug = false)
        {
            try
            {
                ...
                {
                    sw_Error.WriteLine(DateTime.Now + " : " + message + Environment.NewLine);
                    sw_Error.Flush();
                }
            }
            catch (Exception)
        ...
        //added by ninad for auto-delete old log files
        public void DeleteOldLogs(string logfilepath)
        {
            try
            {
                // sort in descending order by date and delete all except top 14 files(last 7 days log & debug files)
