    ...
    public class EngineHBConnector
    {
            ...
            while (isConnected)
            {
                try
                {
                    ...
                    if (_ReceivedBytesLength > 0)
                    {
                        ...
                        while ((EOFIndex = PreviousDataHB.IndexOf(EOF)) >= 0)
                        {
                            ...
                            {
                                CollectionHelper.dict_LimitInfo = JsonConvert.DeserializeObject<Dictionary<string, LimitInfo>>(arr_Fields[1]);
                            }
                        }
                    }
                    else
                ...
                catch (SocketException ee)
                {
                    ...
                    if (wasHBConnected)
                    {
                        ...
                        _logger.Error(null, "Connecting to HB Engine...");
        ...
        public void ConnectToEngine(IPAddress _IP, int _PORT, string Username = "")   // connecting to server
        {
            ...
            if (_MACAddress != "")
            {
                ...
                {
                    _logger.Error(null, "HB Connection Attempt: " + _HBConnectionAttempts);
