...
namespace BOD_Utility
{
    public partial class Home : XtraForm
    {
            ...
            try
            {
                ...
                try
                {
                    ...
                    for (int i = 0; i < arr_Lines.Length; i++)
                    {
                        ...
                        //newLine += arr_Fields[6] + ",";
                        //newLine += arr_Fields[7] + ",";
                        //newLine += arr_Fields[8] + ",";
                        //newLine += arr_Fields[9] + ",";
                        //newLine += arr_Fields[10] + ",";
                        //newLine += arr_Fields[11] + ",";
                        //newLine += arr_Fields[12] + ",";
                        //newLine += arr_Fields[13] + ",";
                        //newLine += arr_Fields[14] + ",";
            ...
            try
            {
                ...
                if (SpanIndex < arr_SpanFileExtensions.Count())
                {
                    ...
                    try
                    {
                        ...
                        DecompressGZAndDelete(new FileInfo(SpanFilePath[0] + SpanFileExactName));
                        filedownloaded = true;
                    }
                    catch (Exception ee) { _logger.Error(ee); }
                    ...
                    try
                    {
                        if (!filedownloaded)
                        {
                            ...
                            using (WebClient webClient = new WebClient())
                            {
                                webClient.DownloadFile(url, SpanFilePath[0] + SpanFileExactName);
                            }

                            using (ZipFile zip = ZipFile.Read(SpanFilePath[0] + SpanFileExactName))
                                zip.ExtractAll(SpanFilePath[0], ExtractExistingFileAction.DoNotOverwrite);

                            File.Delete(SpanFilePath[0] + SpanFileExactName);
                            filedownloaded = true;
                        }
                    }
                    catch (Exception ee) { _logger.Error(ee); }
                    ...
                    try
                    {
                        if (!filedownloaded)
                        {
                            ...
                            File.Copy(filename, SpanFilePath[0] + $"nsccl.{DateTime.Today.ToString("yyyyMMdd")}." + arr_SpanFileExtensions[SpanIndex] + ".spn", true);
                            filedownloaded = true;
                        }
        ...
        private void DownloadCDSpan(Object source, ElapsedEventArgs e, string[] SpanFilePath)
        {
                ...
                if (SpanFilePath[0] != "")
                {
                    ...
                    if (CDSpanIndex < arr_CDSpanFileExtensions.Count())
                    {
                        ...
                        using (WebClient webClient = new WebClient())
                        {
                            ...
                            webClient.DownloadFile(CDSpanFileName, SpanFilePath[0] + SpanFileExactName);
                        }

                        _logger.Debug("CDSpanFileName After : " + CDSpanFileName);

                        var SpanFileNme = DecompressGZAndDelete(new FileInfo(SpanFilePath[0] + SpanFileExactName));
                        if (SpanFileNme != "")
                        {
                            AddToList($"CD Span file [{SpanFileExactName}] downloaded successfully.");

                            string filename = $"nsccl_ix.{DateTime.Today.ToString("yyyyMMdd")}." + arr_SpanFileExtensions[SpanIndex] + ".spn";
            ...
            {
                CDSpanIndex++;
                _logger.Debug("Inside WebException Before Index : " + CDSpanIndex + " CDSpanFileExtensions Count : " + arr_CDSpanFileExtensions.Count());
                ...
                {
                    CDSpanIndex = 0;
                    _logger.Debug("Inside WebException After Index : " + CDSpanIndex + " CDSpanFileExtensions Count : " + arr_CDSpanFileExtensions.Count());
                }
            }
            catch (Exception ee)
            {
            ...
            try
            {
                ...
                try
                {
                    if (!filedownloaded)
                    {
                        ...
                        File.Copy(OTMFileURL[1] + @"\" + OTMFileName, SpanFilePath[0] + OTMFileName, true);
        ...
        // Added by Snehadri on 15JUN2021 for Automatic BOD Process
        private void StartFeedReceivers(string[] CMFeedPath, string[] FOFeedpath, string[] CDFeedPath)
        {
            try
            {
                ...
                try
                {
                    ...
                    if (!list_ComponentStarted.Contains("CDFeedReceiver"))
                    {
                        ...
                        foreach (var cdpath in CDFeedPath)
                        {
                            ...
                            if (connected)
                            {
                                AddToList("CD Feed Receiver Started");
        ...
        private void StartComponents(bool IsRestart = false)            // Added by Snehadri on
        {
            try
            {
                ...
                if (IsWorking && (!list_ComponentStarted.Contains("CDFeedReceiver") && !list_ComponentStarted.Contains("CMFeedReceiver") && !list_ComponentStarted.Contains("FOFeedReceiver")))    // To start the feed receivers 
                {
                    StartFeedReceivers(CMFEEDPath, FOFEEDPath, CDFEEDPath);
                }
