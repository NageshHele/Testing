...
namespace Prime.UI
{
    public partial class uc_UnderlyingClients : XtraUserControl
    {
        ...
        DXMenuItem menu_ExportClientPositionsToCSV = new DXMenuItem();
        ...
        private void InitialiseUC()
        {
            try
            {
                ...
                menu_ExportClientPositionsToCSV.Caption = "Export Clients Positions As CSV";
                menu_ExportClientPositionsToCSV.Click += Menu_ExportClientsPositionsToCSV_Click;
        ...
        private void gv_Client_PopupMenuShowing(object sender, PopupMenuShowingEventArgs e)
        {
            ...
            e.Menu.Items.Add(menu_SaveLayout);
            e.Menu.Items.Add(menu_ExportClientPositionsToCSV);
            // Added by Snehadri on 21OCT2021
            if (e.MenuType == GridMenuType.Column)
            {
                RightClicked_ViewInfo = new GridView(); RightClicked_ViewInfo = e.HitInfo.View;
        ...
        private void Menu_ExportClientsPositionsToCSV_Click(object sender, EventArgs e)
        {
            try
            {
                SaveFileDialog _Save = new SaveFileDialog();
                ...
                if (_Save.ShowDialog() == DialogResult.OK)
                {
                    var view = RightClicked_ViewInfo;
                    view.ExportToCsv(_Save.FileName);

                    //int rc = view.DataRowCount;
                    //for(int i = 0; i < rc; i++)
                    //{
                    //     view.GetRowHandle(i);
