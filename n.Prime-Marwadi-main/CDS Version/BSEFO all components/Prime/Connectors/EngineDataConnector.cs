...
namespace Prime
{
    delegate void del_SpanData(Dictionary<string, double[]> keyValuePairs, string flag);  //Added bool isExpiry Flag Parameter by Akshay on 11-12-2020
    ...
    class EngineDataConnector
    {
        ...
        int _TimeoutMilliseconds = CollectionHelper.TimeoutSeconds * 1000;
        ...
        private void ReceivePrimeResponse(Socket soc_Current)
        {
            ...
            soc_Current.ReceiveTimeout = _TimeoutMilliseconds; 
            ...
            while (isConnected)
            {
                ...
                catch (SocketException ee)
                {
                    ...
                    _logger.Error(ee, "ReceivePrimeResponse Trade Received- " + ProperDataTrade.Length);
        ...
        private void ReceiveSpanResponse(Socket soc_Current)
        {
            ...
            while (isConnected)
            {
                try
                {
                    ...
                    if (_ReceivedBytesLength > 0)
                    {
                        ...
                        while (index >= 0)
                        {
                            ...
                            //getting span marker and removing it
                            var Marker = new byte[3];
                            var final_Buffer = new byte[decomp.Length - Marker.Length];
                            Buffer.BlockCopy(decomp, 0, Marker, 0, 3);
                            Buffer.BlockCopy(decomp, 3, final_Buffer, 0, decomp.Length - 3);

                            var str_marker = Encoding.UTF8.GetString(Marker);
                            var dict_Received = DecompressDeserializeSpan(final_Buffer);

                            if (dict_Received != null)
                                eve_SpanData(dict_Received, Encoding.UTF8.GetString(Marker));
                ...
                catch (SocketException ee)
                {
                    ...
                    _logger.Error(ee, "ReceivePrimeResponse Span Received- " + ProperDataSpan.Length);
