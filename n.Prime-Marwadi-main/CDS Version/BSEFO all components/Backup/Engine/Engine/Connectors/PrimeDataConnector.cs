...
namespace Engine
{
    ...
    class PrimeDataConnector
    {
        ...
        byte[] arr_FOBytes = Encoding.UTF8.GetBytes("FO^");
        byte[] arr_CDBytes = Encoding.UTF8.GetBytes("CD^");
        ...
        static ConcurrentDictionary<string, double[]> dict_MarginDataCDS = new ConcurrentDictionary<string, double[]>();
        ...
        public void SendSpanData(Dictionary<string, double[]> dict_AllMarginData, Dictionary<string, double[]> dict_AllMarginDataCDS)
        {
            try
            {
                dict_MarginData = new ConcurrentDictionary<string, double[]>(dict_AllMarginData);
                dict_MarginDataCDS = new ConcurrentDictionary<string, double[]>(dict_AllMarginDataCDS);
                
                if (dict_MarginData.Any()||dict_MarginDataCDS.Any())
                {
        ...
        private void SendSpanToPrime(string Username)
        {
            try
            {
                ...
                if (dict_ConnectedPrimeSpan.TryGetValue(Username, out _Socket) && dict_UserMappedInfo.TryGetValue(Username, out string[] arr_MappedClients))
                {
                    ...
                    //ADDING SPAN MARKER
                    byte[] final_Buffer = new byte[arr_FOBytes.Length + buffer.Length];
                    Buffer.BlockCopy(arr_FOBytes, 0, final_Buffer, 0, arr_FOBytes.Length);
                    Buffer.BlockCopy(buffer, 0, final_Buffer, arr_FOBytes.Length, buffer.Length);
                    //MARKER END

                    if (final_Buffer != null)
                    {
                        _Socket.Send(final_Buffer, final_Buffer.Length, SocketFlags.None);
                    }
                    Thread.Sleep(50);


                    //CDS
                    dict_Filtered = dict_MarginDataCDS.Where(x => arr_MappedClients.Contains(x.Key.Split('_')[0])).ToDictionary(x => x.Key, x => x.Value);
                    buffer = SerializeCompressSpan(dict_Filtered);

                    //ADDING SPAN MARKER
                    final_Buffer = new byte[arr_CDBytes.Length + buffer.Length];
                    Buffer.BlockCopy(arr_CDBytes, 0, final_Buffer, 0, arr_CDBytes.Length);
                    Buffer.BlockCopy(buffer, 0, final_Buffer, arr_CDBytes.Length, buffer.Length);
