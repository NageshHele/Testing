...
namespace Engine
{
    class BODUtilityConnector                                          // Added by Snehadri on 30JUN2021 for Automatic BOD Process
    {
        ...
        public bool StartClient()
        {
            try
            {
                _logger = new clsWriteLog(Application.StartupPath + "\\Log");
                ...
                try
                {
                    while (!IsStarted)
                    {
                        ...
                        byte[] msg = Encoding.ASCII.GetBytes("EngineStarted");
                ...
                {
                    sender.Close();
                    _logger.WriteLog("Sending Message Error in BODUtilityConnector: " + error.ToString());
                    return false;
            ...
            {
                _logger.WriteLog("Socket Error in BODUtilityConnector: " + error.ToString());
                return false;
