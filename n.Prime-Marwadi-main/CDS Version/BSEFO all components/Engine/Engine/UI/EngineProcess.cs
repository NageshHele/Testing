...
namespace Engine
{
    public partial class EngineProcess : XtraForm
    {
            ...
            try
            {
                    ...
                    if (dict_UnderlyingToken.TryGetValue(UnderlyingNameKey, out int CurrentMonthUnderlyingToken))
                    {
                        ...
                        if (!dict_LTP.ContainsKey(CurrentMonthUnderlyingKey))
                        {
                            ...
                            else if (_PositionInfo.Segment == en_Segment.BSECM)
                            {
                                _FeedLibrary.BSECMSubscribe(CurrentMonthUnderlyingToken);
                            }   
                    ...
                    //if (dict_SpotTokenInfo.TryGetValue(Underlying, out List<string> _SpotTokens))
                    //{
                    ...
                    //    else
                    //    {
                    //        if(_SpotToken.Split('|')[0] == "NSECM")
                    //        {
                    //            _FeedLibrary.CMSubscribe(Convert.ToInt32(_SpotToken.Split('|')[1]));
                    //        }
                    //        else if (_SpotToken.Split('|')[0] == "BSECM")
                    //        {
                    //            _FeedLibrary.BSECMSubscribe(Convert.ToInt32(_SpotToken.Split('|')[1]));
                    //        }                          
                ...
                var SpotUnderlyingSegment = (ScripInfo.Segment == en_Segment.NSEFO || ScripInfo.Segment == en_Segment.NSECM) ? en_Segment.NSECM : en_Segment.BSECM;
                if (dict_SpotTokenInfo.TryGetValue(SpotUnderlyingSegment+"|"+Underlying, out string _SpotToken))
                {
                    if (dict_LTP.TryGetValue(_SpotToken, out arr_LTP))
                    {
                        if (arr_LTP[0] != -1)
                    ...
                    {
                        if (SpotUnderlyingSegment == en_Segment.NSECM)
                        {
                            _FeedLibrary.CMSubscribe(Convert.ToInt32(_SpotToken.Split('|')[1]));
                            dict_LTP.TryAdd(_SpotToken, new double[LTPArray] { -1, -1, 0 });
                        }
                        else if (SpotUnderlyingSegment == en_Segment.BSECM)
                        {
                            _FeedLibrary.BSECMSubscribe(Convert.ToInt32(_SpotToken.Split('|')[1]));
                            dict_LTP.TryAdd(_SpotToken, new double[LTPArray] { -1, -1, 0 });
                        }
                ...
                if (_ScripType == n.Structs.en_ScripType.CE || _ScripType == n.Structs.en_ScripType.PE)
                {
                        ...
                        var time = _PositionInfo.ExpiryTimeSpan.TotalDays;
        ...
        private void ReadClosingFile()
        {
            ...
            var dict_IndexCloseBSE = new Dictionary<string, double>();

            //added by nikhil for bse index closing
            try
            {
                var _Closingfile = new DirectoryInfo("C:/Prime").GetFiles("index*_*").OrderByDescending(x => x.LastWriteTime).FirstOrDefault();
                if (_Closingfile == null)
                {
                    _logger.WriteLog("BSE Index closing file not found");
                    return;
                }
                ...
                foreach (var line in arr_lines)
                {
                    ...
                    {
                        var Arr_Fields = line.Split('|');
                        if (!dict_IndexCloseBSE.ContainsKey(Arr_Fields[0]))
                        {
                            dict_IndexCloseBSE.Add(Arr_Fields[0], Convert.ToDouble(Arr_Fields[1]) / 100);
                        }
                    }
                    catch (Exception ee) { _logger.WriteLog(ee.ToString()); }
            ...
            try
            {
                var arr_Lines = File.ReadAllLines("C:/Prime/IndexTokens.csv");
                ...
                foreach (var Line in arr_Lines)
                {
                    try
                    {
                        ...
                        {
                            if (dict_IndexCloseBSE.TryGetValue(arr_Fields[4], out double Closing))
                            {
                                dict_ScripClosing.TryAdd($"{arr_Fields[0]}",Closing);
                            }
                        }
                    }
                    catch (Exception ee) { _logger.WriteLog(ee.ToString()); }
            ...
            try
            {
                ...
                foreach (ds_Engine.dt_ContractMasterRow v in dt_ContractMaster.Rows)
                {
                    ...
                    if (ScripInfo.Segment == en_Segment.NSECM) //&& ScripInfo.Series == "EQ")
                    {
                        ...
                        {
                            if (ScripInfo.Series == "EQ")
                                dict_SpotTokenInfo.TryAdd($"{ScripInfo.Segment}|{ScripInfo.Symbol}", $"{ScripInfo.Segment}|{ScripInfo.Token}");
                        }
                    ...
                    else if(ScripInfo.Segment == en_Segment.BSECM)
                    {
                        if (ScripInfo.Series == "EQ")
                            dict_SpotTokenInfo.TryAdd($"{ScripInfo.Segment}|{ScripInfo.Symbol}", $"{ScripInfo.Segment}|{ScripInfo.Token}");
            ...
            try
            {
              ...
              $"_{isSpanConnected}_{isCDSSpanConnected}_{dict_IndexLTP["NIFTY"]}_{dict_IndexLTP["BANKNIFTY"]}_{_SpanComputeTime}_{_CDSSpanComputeTime}_{_LatestSpanFileName}_{_LatestCDSSpanFileName}")); //added by Akshay on 16-11-2021 for CD LTT
                ...
                if (dict_Greeks.TryGetValue(LTPKey, out Greeks _GreeksInfo))
                {
                    if (_BSEFOPacket.IV != "-")
                    {
                        _logger.WriteLog("IV | " + _BSEFOPacket.IV);

                        //added Rounding on 01DEC2020 by Amey
                        double ReceivedIV = Math.Round(Convert.ToDouble(_BSEFOPacket.IV), 2);
                        double Lower = ReceivedIV / _IVDivisor;
                        double Higher = ReceivedIV * _IVMultiplier;
                    ...
                    if (_BSEFOPacket.Delta != "-")
                        _GreeksInfo.Delta = Convert.ToDouble(_BSEFOPacket.Delta);
                    if (_BSEFOPacket.Gamma != "-")
                        _GreeksInfo.Gamma = Convert.ToDouble(_BSEFOPacket.Gamma);
                    if (_BSEFOPacket.Theta != "-")
                        _GreeksInfo.Theta = Convert.ToDouble(_BSEFOPacket.Theta);
                    if (_BSEFOPacket.Vega != "-")
                        _GreeksInfo.Vega = Convert.ToDouble(_BSEFOPacket.Vega);
                }
