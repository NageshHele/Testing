...
namespace Gateway
{
    ...
    public delegate void del_TradeFileStatus(bool isFOAvailable, bool isBSEFOAvailable);
    ...
    public class nProcess
    {
        ...
        private void Instance_eve_TradeFileStatus(bool isFOAvailable, bool isBSEFOAvailable)
        {
            eve_TradeFileStatus(isFOAvailable, isBSEFOAvailable);
        }

        #region TradeProcess Events

        private void Instance_eve_TradeTime(double _FOLastTradeTime, double _BSEFOLastTradeTime)
        {
        ...
        private void InitialiseSpan()
        {
            try
            {
                var list_NSEFOEOD = list_EODPositions.Where(v => v.Segment == en_Segment.NSEFO || v.Segment==en_Segment.BSEFO).ToList();
        ...
        private void ConsolidatePositions(List<PositionInfo> list_RecvPosition, bool isEODPosition = false)
        {
            try
            {
                ...
                foreach (var _Position in list_RecvPosition)
                {
                    ...
                    var _ScripInfo = dict_TokenScripInfo[$"{_Position.Segment}|{_Position.Token}"];
