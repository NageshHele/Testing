...
namespace Gateway.Core_Logic
{
    ...
    class CTradeProcess
    {
        ...
        internal List<PositionInfo> GetTrades(bool ClearTrades)
        {
            ...
            ReadFO();
            //Parallel.Invoke(() => ReadFO(), () => ReadCM(), () => ReadBSECM(), () => ReadCP());//, () => ReadBSECM(), () => ReadGiveUpTrades());
                            ...
                            using (StreamReader sr = new StreamReader(stream))
                            {
                               ...
                               // var PrevLineCounter = 0;    //Added by Akshay on 01-09-2021 for new trade reading logic
                                ...
                                while ((strData = sr.ReadLine()) != null)
                                {
                                    ...
                                    {
                                        _logger.Error("Inconsistent line found | " + strData);
                                        break;
                                    }
                                    ...
                                    if (arr_Fields.Length > 22 && arr_Fields[11] != "")
                                    {
                                        ...
                                        for (int i = 0; i < arr_Combinations.Length; i++)
                                        {
                                            try
                                            {
                                                ...
                                                Symbol = arr_Fields[3].Trim().ToUpper();

                                                string _check = TradeID + "_" + ClientCode + "_" + Symbol;
                                                if (dict_RandomEntriesBSECM[FullFilePath].Contains(_check))
                                                {
                                                    _logger.Error("ReadBSECM Skipped Check : " + _check, true);
                                                    continue;
                                                }
                                                ...
                                                else if (dict_BSEMapping.ContainsKey(Symbol))
                                                {
                                                    var CnvrtScripname = dict_BSEMapping[Symbol] + "-EQ";

                                                    if (dict_ScripInfo.ContainsKey(en_Segment.NSECM + "|" + CnvrtScripname))
                                                        _ScripInfo = dict_ScripInfo[en_Segment.NSECM + "|" + CnvrtScripname];
                                                }
