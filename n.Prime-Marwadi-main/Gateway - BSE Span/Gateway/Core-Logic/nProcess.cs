    ...
    public class nProcess
    {
            ...
            string _SpanFolderPath)
        {
            try
            {
                ...
                CTradeProcess.Initialise(_logger, ds_Config, isDebug, dict_CustomScripInfo, dict_ScripInfo, dict_TokenScripInfo, dict_BSEMapping);
            ...
            try
            {
                ...
                foreach (var Line in arr_Lines)
                {
                    try
                    {
                        ...
                        if (!dict_BSEMapping.ContainsKey(_NSESymbol))
                            dict_BSEMapping.Add(_NSESymbol, _BSESymbol);
                    }
                    catch (Exception ee) {
        ...
        private void ReceiveResponseFromEngine(string _Signal)
        {
            try
            {
                ...
                if (_Signal.Equals("START"))
                {
                    ...
                    LatestSpanFileName = SpanDirectory.GetFiles("BSERISK*.XML").Any() ?
                                SpanDirectory.GetFiles("BSE*.XML")
                    ...
                    for (int EODIdx = 0; EODIdx < list_EODPositions.Count; EODIdx++)
                    {
                        try
                        {
                            ...
                            if (dict_TokenScripInfo.TryGetValue(ScripKey, out ContractMaster ScripInfo) && 
                                dict_BSEMapping.TryGetValue(ScripInfo.Symbol, out string _BSEUnderlying))
                            {
                                ...
                                if (ScripInfo.ScripType != en_ScripType.EQ)
                                {
                                    ...
                                    if (isSpanConnected)
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                arr_Margin = _SpanConnector.GetMargin(1, SpanKey, "BSE", "EQFO", _BSEUnderlying, dtExpiry.ToString("yyyy-MM-dd").Replace("-", ""), "", "", "E", _PositionInfo.TradeQuantity.ToString());
                                                arr_EODMargin = _SpanConnector.GetMargin(1, EODKey, "BSE", "EQFO", _BSEUnderlying, dtExpiry.ToString("yyyy-MM-dd").Replace("-", ""), "", "", "E", _PositionInfo.TradeQuantity.ToString());

                                                //Changed on 22JAN2021 by Akshay
                                                if (dtNearestExpiry < dtExpiry)  //Changed on 22JAN2021 by Akshay
                                                {
                                                    arr_ExpiryMargin = _SpanConnector.GetMargin(1, ExpiryKey, "BSE", "EQFO", _BSEUnderlying, dtExpiry.ToString("yyyy-MM-dd").Replace("-", ""), "", "", "E", _PositionInfo.TradeQuantity.ToString());
                                            ...
                                            {
                                                arr_Margin = _SpanConnector.GetMargin(1, SpanKey, "BSE", "EQFO", _BSEUnderlying, dtExpiry.ToString("yyyy-MM-dd").Replace("-", ""), ScripInfo.StrikePrice.ToString("N2").Replace(",", ""), ScripInfo.ScripType.ToString().Substring(0, 1), "E", _PositionInfo.TradeQuantity.ToString());
                                                arr_EODMargin = _SpanConnector.GetMargin(1, EODKey, "BSE", "EQFO", _BSEUnderlying, dtExpiry.ToString("yyyy-MM-dd").Replace("-", ""), ScripInfo.StrikePrice.ToString("N2").Replace(",", ""), ScripInfo.ScripType.ToString().Substring(0, 1), "E", _PositionInfo.TradeQuantity.ToString());

                                                //Changed on 22JAN2021 For Nearest expirydate
                                                if (dtNearestExpiry < dtExpiry)  //Changed on 22JAN2021 For Nearest expirydate
                                                {
                                                    arr_ExpiryMargin = _SpanConnector.GetMargin(1, ExpiryKey, "BSE", "EQFO", _BSEUnderlying, dtExpiry.ToString("yyyy-MM-dd").Replace("-", ""), ScripInfo.StrikePrice.ToString("N2").Replace(",", ""), ScripInfo.ScripType.ToString().Substring(0, 1), "E", _PositionInfo.TradeQuantity.ToString());
                        ...
                        using (MySqlDataReader _mySqlDataReader = myCmd.ExecuteReader())
                        {
                            while (_mySqlDataReader.Read())
                            {
                                try
                                {
                                    ...
                                    {
                                        if (dict_ClientInfo.ContainsKey($"{ClienCode}|{CTCL_ID}|{UserID}"))
                                            dict_ClientInfo[$"{ClienCode}|{CTCL_ID}|{UserID}"].Add(Username);
                                        else
                                            dict_ClientInfo.TryAdd($"{ClienCode}|{CTCL_ID}|{UserID}", new HashSet<string> { Username });
                                    }
                                    else if (ClienCode != "")
                                    {
                                        if (dict_ClientInfo.ContainsKey(ClienCode))
                                            dict_ClientInfo[ClienCode].Add(Username);
                                        else
                                            dict_ClientInfo.TryAdd(ClienCode, new HashSet<string> { Username });
                                    }
                                    else if (CTCL_ID != "")
                                    {
                                        if (dict_ClientInfo.ContainsKey(CTCL_ID))
                                            dict_ClientInfo[CTCL_ID].Add(Username);
                                        else
                                            dict_ClientInfo.TryAdd(CTCL_ID, new HashSet<string> { Username });
                                    }
                                    else if (UserID != "")
                                    {
                                        if (dict_ClientInfo.ContainsKey(UserID))
                                            dict_ClientInfo[UserID].Add(Username);
                                        else
                                            dict_ClientInfo.TryAdd(UserID, new HashSet<string> { Username });
                                    }
                                }
                                catch (Exception) { }
                            }
        ...
        internal void ReloadAndRecalculateMargin(bool isButtonClick = false)
        {
            ...
            if (isSpanConnected)
            {
                ...
                try
                {
                    ...
                    var SpanDirectory = new DirectoryInfo(_SpanFolderPath);
                    LatestSpanFileName = SpanDirectory.GetFiles("BSERISK*.XML").Any() ?
                                SpanDirectory.GetFiles("BASERISK*.XML")
                               .OrderByDescending(f => f.LastWriteTime)
                               .First().Name : "";
